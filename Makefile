.PHONY: review fmt-check lint test audit deny coverage mutants review-quick wheel docs docs-serve docs-toolchain
...
# Build a wheel into target/wheels; try it with `uvx --from target/wheels/<wheel> dermestes --all`
wheel:
